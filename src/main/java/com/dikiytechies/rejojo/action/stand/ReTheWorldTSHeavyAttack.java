...
public class ReTheWorldTSHeavyAttack extends StandEntityAction implements IHasStandPunch {
    ...
    public ReTheWorldTSHeavyAttack(StandEntityAction.Builder builder,
                                   Supplier<StandEntityHeavyAttack> theWorldHeavyAttack, Supplier<ReTimeStopInstant> theWorldTimeStopBlink) {
    ...
    protected Action<IStandPower> replaceAction(IStandPower power, ActionTarget target) {
        if (power.getStandManifestation() instanceof StandEntity) {
            ...
            if (stand.getCurrentTaskPhase().isPresent() && (stand.getCurrentTaskPhase().get() == Phase.PERFORM || stand.getCurrentTaskPhase().get() == Phase.RECOVERY)) {
                return ModStandsReInit.THE_WORLD_TS_BLINK_RECOVERY.get();
            }
        }
    ...
    protected void preTaskInit(World world, IStandPower standPower, StandEntity standEntity, ActionTarget target) {
        ...
        if (!world.isClientSide() || standEntity.isManuallyControlled()) {
            ...
            if (aimingEntity != null) {
                ReTimeStopInstant blink = theWorldTimeStopBlink.get();
                ...
                int timeStopTicks = ReTimeStop.getTimeStopTicks(standPower, timeStop);
                ...
                if (standEntity.getAttributeValue(Attributes.MOVEMENT_SPEED) > 0) {
                    ...
                    double ticksForDistance = pos.subtract(standEntity.position()).length() / ReTimeStopInstant.getDistancePerTick(standEntity);
                ...
                ReTimeStopInstant.skipTicksForStandAndUser(standPower, timeStopTicks);
                if (!world.isClientSide()) {
    ...
    public StandEntityPunch punchEntity(StandEntity stand, Entity target, StandEntityDamageSource dmgSource) {
        ...
        return new ReTheWorldTSHeavyAttack.ReTheWorldTSHeavyPunch(stand, target, dmgSource)
    ...
    public static class ReTheWorldTSHeavyPunch extends StandEntityHeavyAttack.HeavyPunchInstance {
        private Supplier<SoundEvent> voiceLineOnKill = () -> null;

        public ReTheWorldTSHeavyPunch(StandEntity stand, Entity target, StandEntityDamageSource dmgSource) {
            super(stand, target, dmgSource);
        }

        public ReTheWorldTSHeavyPunch setVoiceLineOnKill(Supplier<SoundEvent> sound) {
